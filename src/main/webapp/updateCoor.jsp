	...
	<section class="area-login">
		<div class="login">
			...
			<form method="GET" action="edite" name="frmCoord">
				<input value="<%=coord.getId() %>" type="hidden" name="id" required> 
				<input value="<%=coord.getNome() %>" type="text" name="nome" placeholder="Informe o nome do coordenador" autofocus required> 
				<select id="curso" name="cursos" multiple required>
				...
				<select id="periodo" name="periodos" multiple required>
				    <% for (int i = 0; i < listaPeriodos.size(); i++) { %>
				        <% Periodos periodo = listaPeriodos.get(i); %>
				        <option value="<%=periodo.getId()%>"><%=periodo.getDia() + " - " + periodo.getHorario()%></option>
				    <% } %>
				</select>
